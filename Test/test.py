...
class PrimitiveTypesCase(unittest.TestCase):
    ...
    def dumps_and_loads(self):
        self.assertEqual(self.json_serializer.dumps('abc'), '{"VALUE": "abc", "TYPE": "str"}')
        self.assertEqual(self.xml_serializer.dumps('abc'), {"<class 'str'>": 'abc'})

        self.assertEqual(self.json_serializer.dumps(78), '{"VALUE": "78", "TYPE": "int"}')
        self.assertEqual(self.xml_serializer.dumps(78), {"<class 'int'>": 78})
...
class CollectionsCase(unittest.TestCase):
    ...
    def test_collections(self):
        test_list = [
            '1', 3, 123.123, 'None', True, False
        ]

        self.assertEqual(self.json_serialize.loads(self.json_serialize.dumps(test_list)), test_list)

        self.assertEqual(self.xml_serialize.loads(self.xml_serialize.dumps(test_list)), test_list)

        with open('Files/JSON.json', 'w') as fw:
            self.json_serialize.dump(test_list, 'Files/JSON.json')
...
class ClassCase(unittest.TestCase):
    ...
    def test_meta(self):
        self.assertEqual(self.json_serializer.loads(self.json_serializer.dumps(B().my_attr)), B().my_attr)
